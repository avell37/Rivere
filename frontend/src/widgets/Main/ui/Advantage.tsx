...
export const Advantage = ({ title, text, icon, iconBg }: AdvantageProps) => {
	return (
		<div
			className='flex flex-col items-center gap-4 rounded-2xl border border-main-card p-8 
		...
		>
			...
			<p className='opacity-80 text-white'>{text}</p>
		</div>
	)
}
...
export const AdvantageWithStep = ({
	...
	return (
		<div
			className='flex flex-col gap-4 rounded-2xl border border-main-card p-8 
			shadow-[0_0_10px_rgba(180,0,255,0.2)] hover:shadow-[0_0_20px_rgba(180,0,255,0.4)] 
			transition duration-400 hover:-translate-y-2 cursor-pointer'
		>
			{step && stepIcon && (
				<div className='flex flex-col items-center gap-2'>
					<span className='text-3xl border p-2 px-4.5 rounded-full text-white'>
						{step}
					</span>
					{stepIcon}
				</div>
